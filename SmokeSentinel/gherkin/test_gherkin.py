...
class TestGenerator:
    ...
    def _make_generator(self) -> GherkinGenerator:
        """Create a GherkinGenerator with a dummy API key (LLM will be mocked)."""
        with patch.dict("os.environ", {"ANTHROPIC_API_KEY": "sk-ant-test-key"}):
            return GherkinGenerator(api_key="sk-ant-test-key")

    def test_run_success(self, tmp_path):
        gen = self._make_generator()
        gen._call_llm = MagicMock(return_value=VALID_GHERKIN)
    ...
    def test_retry_on_invalid_output(self, tmp_path):
        ...
        gen._call_llm = MagicMock(side_effect=["not valid gherkin at all", VALID_GHERKIN])
    ...
    def test_raises_after_max_retries(self, tmp_path):
        gen = self._make_generator()
        gen._call_llm = MagicMock(return_value="not valid gherkin at all")
